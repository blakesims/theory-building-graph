...
def main():
    ...
    names = sorted({name for c in selected for m in maps[c['id']] for name in m.get('tests', [])})
    ...
    for c in selected:
        ...
        cases.append({'id': c['id'], 'title': c['title'], 'mechanism': c['mechanism'], 'status': status, 'tests': outcomes, 'coverage_claims': mapped})
    ...
    report = {'suite_version': spec['suite_version'], 'case_count': len(cases), 'counts': counts,
              'all_passed': bool(cases) and all(c['status']=='passed' for c in cases),
    ...
    print(json.dumps({'counts': counts, 'all_passed': report['all_passed'], 'tests': len(names), 'report': str(args.report)}, indent=2))
