...
import sys as _sys; from pathlib import Path as _Path; _ROOT=str(_Path(__file__).resolve().parents[1]); _sys.path.insert(0,_ROOT) if _ROOT not in _sys.path else None
import json
import re
from pathlib import Path
from acceptance import session_fixtures
...
def prepare(output):
 ...
 contract=(Path(__file__).resolve().parents[1]/'docs'/'AGENT_CONTRACT.md').read_text()
