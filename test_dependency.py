...
class IntegrationTests(unittest.TestCase):
    def test_compact_json_read_keeps_semantics(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            base=[sys.executable,str(Path(g.__file__)),'--file',str(path),'node','A','--json']
...
class CanonicalTests(unittest.TestCase):
    def test_canonical_export_and_rename(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            output=[subprocess.check_output([sys.executable,g.__file__,'--file',str(p),'export']) for p in paths]
...
class AcceptanceIntegrationTests(unittest.TestCase):
    ...
    def test_I07_every_semantic_input_and_unrelated_negative(self):
        from test_tracecheck import TraceChecks
        import tracecheck
