...
class FableDependencyRegressions(unittest.TestCase):
 ...
 def test_N3_question_requirement_typos_and_legacy_reject_atomically(self):
  for required in ['answerd','accepted']:
   with self.subTest(required=required):
    model=fixture();model['node_types']['question']['states']=['open','answered','retired'];self.put(model);before=self.path.read_bytes()
    ...
    with self.assertRaisesRegex(graph.GraphError,'B-depends-on-Q.*question declared states'):
 ...
 def test_N3_partial_question_resolution_is_valid_requirement(self):
  model=fixture();model['node_types']['question']['states']=['open','partial-answer','answered'];model['nodes']['Q']['status']='partial-answer';edge(model,'B','Q',requires=['partial-answer']);edge(model,'A','Q','answers',coverage='partial')
 ...
 def test_N3_declared_custom_status_vocabulary_respected(self):
  ...
  self.assertEqual(self.path.read_bytes(),before)
 def test_P1_review_and_content_edits_leave_dependents_current(self):
  model=fixture();edge(model,'B','A');edge(model,'Q','B');self.put(model)
  self.edit('A',text='new premise')
  self.edit('Q',meta={'review_state':'current'})
  third=self.edit('B',meta={'review_state':'current'})
  self.assertEqual(dependency.currency(third['nodes']['Q']),'current')
  self.assertEqual(dependency.currency(self.edit('B',text='changed consequence')['nodes']['Q']),'current')
 def test_P2_custom_edge_add_remove_leaves_dependents_current(self):
  for action in ['add','delete']:
   with self.subTest(action=action):
    ...
    self.assertEqual(dependency.currency(after['nodes']['B']),'current');self.assertEqual(dependency.currency(after['nodes']['Q']),'current')
 def test_P3_question_prerequisite_uses_declared_status(self):
  model=fixture();model['nodes']['B'].update(type='question',status='answered');edge(model,'Q','B',requires=['answered'])
  self.assertEqual(dependency.resolution(model,'B')['resolution'],'answered');self.assertEqual(dependency.readiness(model,'Q')['readiness'],'ready')
  ...
  model['nodes']['B']['status']='answered';model['nodes']['A']['meta']={'review_state':'needs-review'}
 ...
 def test_P26_null_meta_rejected_atomically(self):
  ...
  self.assertEqual(self.path.read_bytes(),before)
 def test_P28_declared_answer_has_no_drift_finding(self):
  model=fixture();model['nodes']['Q']['status']='answered'
  self.assertNotIn('answer-state-drift',graph.check(model)['counts'])
...
class FableScopeAndPresentationRegressions(unittest.TestCase):
 def test_H2_default_question_requirement_is_answered(self):
  ...
  self.assertEqual(dependency.readiness(model,'Q')['readiness'],'blocked')
  edge(model,'A','B','answers',coverage='full');self.assertEqual(dependency.readiness(model,'Q')['readiness'],'blocked')
  model['nodes']['B']['status']='answered';self.assertEqual(dependency.readiness(model,'Q')['readiness'],'ready')
