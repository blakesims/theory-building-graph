...
def compact(data, full=False):
    ...
    if 'changes' in data and isinstance(data['changes'],list):
        ...
        return '\n'.join(lines)
    if 'findings' in data:
        return '\n'.join([f"Revision {data['revision']} · {len(data['findings'])} findings"]+[f"{f['code']}: {f['message']} [{', '.join(f['nodes'])}]" for f in data['findings']]+[data['scope']])
    if not isinstance(data.get('nodes'),dict): return '\n'.join(f'{k}: {display(v)}' for k,v in data.items())
    lines=[' · '.join(f'{k}={display(v)}' for k,v in data.items() if k not in ('nodes','edges','distances','question_states'))]
...
def serve(path,port):
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            try:
                route=parsed.path
                if route in ('/','/index.html'): return self.send((HERE/'index.html').read_bytes(),'text/html; charset=utf-8')
                if route=='/vendor/cytoscape.min.js': return self.send((HERE/'vendor'/'cytoscape.min.js').read_bytes(),'application/javascript')
                g=load(path)
...
def main():
    ...
    while i<len(argv):
        ...
        elif argv[i]=='--file': front.extend(argv[i:i+2]);i+=1
        elif argv[i].startswith('--file='): front.append(argv[i])
