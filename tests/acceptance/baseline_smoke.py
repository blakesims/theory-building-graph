...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--engine',type=Path,default=Path(__file__).resolve().parents[2]);a=p.parse_args();engine=a.engine.resolve();original=engine/'projects'/'morphisms'/'graph.json';raw=original.read_bytes()
