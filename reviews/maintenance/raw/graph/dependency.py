...
def readiness(g, nid):
    ...
    def visit(node, stack):
        ...
        for eid, target in edges:
            ...
            elif n.get('status') not in edge.get('requires', ['accepted']): reasons.append('not-accepted')
...
def findings(g):
    ...
    for nid, n in sorted(g['nodes'].items()):
        ...
        if n['type'] == 'question':
            ...
            if n.get('status') in {'answered', 'complete'} and resolution(g, nid)['resolution'] != 'answered': add('answer-state-drift', nid, 'Stored completion has no current full accepted answer of the required shape.', 'review')
        if n['type'] == 'claim' and n.get('status') == 'accepted' and not any(g['nodes'][e['to'] if e['from'] == nid else e['from']]['type'] in {'case', 'trace'} for _, e in refs):
            add('untested-claim', nid, 'Accepted claim has no directly linked case; this is a coverage gap.')
...
def validate(g):
    ...
    for eid, e in dependency_edges(g):
        if 'requires' in e and (not isinstance(e['requires'], list) or not e['requires']): raise ValueError('requires must be nonempty status list: ' + eid)
...
def refresh(original, g, edits, reviewed):
    ...
    for edit in list(edits):
        ...
        if edit['collection'] == 'nodes' and before and after:
            if fingerprint(before) != fingerprint(after) or currency(before) != currency(after):
                roots.add(key)
            ...
            for edge in (before, after):
                ...
                if edge['type'] in {'depends-on', 'extracted-from'}:
