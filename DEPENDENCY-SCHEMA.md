...
`requires` defaults to accepted. The default is mandatory all-of. Edges with the same nonempty `any_group` form an alternative group: any one available/current qualifying premise satisfies that group. Distinct groups and ungrouped edges are required together. Withdrawn or historical premises remain blockers; they are not filtered away. `answers`, `about`, `supports`, and `revises` never become logical dependencies automatically.
