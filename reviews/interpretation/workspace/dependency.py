...
def refresh(original, g, edits, reviewed):
    ...
    for edit in list(edits):
        ...
        if edit['collection'] == 'nodes' and before and after:
            if fingerprint(before) != fingerprint(after) or (currency(before) != currency(after) and currency(after) != 'current'):
                roots.add(key)
            ...
            semantic_fields=('invalidation','readiness')
            if any((before or {}).get(k)!=(after or {}).get(k) for k in semantic_fields):
