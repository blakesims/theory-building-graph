...
with tempfile.TemporaryDirectory() as d:
 ...
 def run(*args):
  r=subprocess.run(['python3',str(engine/'graph.py'),'--file',str(graph),'--json',*args],text=True,capture_output=True)
...
print(json.dumps({'baseline':'passed','scope':'7 current-checker outcome assertions; not80-case compliance','graph_revision':json.loads(raw)['revision'],'engine_sha256':hashlib.sha256((engine/'graph.py').read_bytes()).hexdigest(),'outcomes':outcomes,'live_graph_unchanged':True},indent=2))
