...
def scope_overlap(a, b, model):
    ...
    if a not in scopes or b not in scopes:
        missing = sorted({s for s in (a, b) if s not in scopes}, key=str)
        return result('not-checked', missing_scopes=missing,
                      reason='Undeclared scope IDs: ' + ', '.join(repr(s) for s in missing) +
                      '. Declare their justified semantics in formal_model.scopes. Use explicit members or justified nonempty evidence and containment/overlap facts; do not invent an all-scope witness.')
...
def compare_authority(a, b, model):
    ...
    if 'only' in (ma, mb):
        ...
        if positive.get('modality') == 'never' and relation == 'same':
            return result('no-inconsistency-established',
                          reason='eligible actors are prohibited, but no occurrence is required',
                          possible_overconstraint=True, witness=witness)
